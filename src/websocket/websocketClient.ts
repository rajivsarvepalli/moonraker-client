...
export class Websocket {
  ...
  private _closedByUser = false; // whether the websocket was closed by the user
   ...
   */
  private handleOpenEvent = (event: WebSocket.Event) =>
   ...
   * @param event to handle.
   */
  private handleErrorEvent = (event: WebSocket.Event) =>
    ...
    switch (type) {
      ...
      case WebsocketEvent.open:
        if (this.backoff !== undefined && this._lastConnection !== undefined) {
          ...
          const event: ReconnectEventDetail = detail;
  ...
  private scheduleConnectionRetryIfNeeded() {
    ...
    const handleRetryEvent = (detail: RetryEventDetail) => {
      const event: RetryEventDetail = detail;
